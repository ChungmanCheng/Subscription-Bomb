...
def main() -> None:
    while True:
        # Reload on every menu pass so the previous operation's changes are
        # immediately visible instead of showing only the startup totals.
        verified   = load_subscription_urls(verified_only=True)
        unverified = load_subscription_urls(unverified_only=True)
        print(f"\nCurrent verified URLs  : {len(verified)}")
