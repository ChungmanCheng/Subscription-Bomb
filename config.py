...
_url_json_setting = os.getenv("URL_JSON", "email_subscription.json").strip()
if not _url_json_setting:
    _url_json_setting = "email_subscription.json"
URL_JSON: str = (
    _url_json_setting
    if os.path.isabs(_url_json_setting)
    else os.path.join(PROJECT_DIR, _url_json_setting)
)
...
# Search widely enough to backfill candidates that are blocked or unsupported.
AUTO_RESULTS_PER_QUERY: int = _env_int("AUTO_RESULTS_PER_QUERY", 20, 1, 20)
AUTO_MAX_URLS: int = _env_int("AUTO_MAX_URLS", 50, 1, 500)
AUTO_TARGET_ADDITIONS: int = _env_int(
    "AUTO_TARGET_ADDITIONS", AUTO_MAX_URLS, 1, 500
)
AUTO_MAX_CANDIDATES: int = _env_int(
    "AUTO_MAX_CANDIDATES", max(500, AUTO_TARGET_ADDITIONS * 8),
    AUTO_TARGET_ADDITIONS, 2000,
)
AUTO_EXPAND_SEARCH_QUERIES: bool = _env_bool("AUTO_EXPAND_SEARCH_QUERIES", True)
AUTO_MIN_SEARCH_SCORE: float = _env_float("AUTO_MIN_SEARCH_SCORE", 0.0, 0.0, 1.0)
AUTO_FOLLOW_LINKS: bool = _env_bool("AUTO_FOLLOW_LINKS", True)
AUTO_LINKS_PER_PAGE: int = _env_int("AUTO_LINKS_PER_PAGE", 3, 0, 20)
AUTO_RESPECT_ROBOTS: bool = _env_bool("AUTO_RESPECT_ROBOTS", False)
